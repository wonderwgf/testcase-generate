...
- 支持Story ID标签和前置条件备注
- 默认逻辑图向右布局
- 自动识别优先级（从 Markdown 字段读取）

依赖：pip install xmind

使用方式：
1. 命令行：python markdown_to_xmind.py <md文件路径> [输出目录]
2. 代码调用：create_xmind_from_md("output/test_cases/详细测试用例.md", "output/xmind")
...
def create_xmind_from_md(
    ...
    参数：
        ...
        custom_path (str): XMind 根节点标题，默认从文件名或 md 首行 # 标题推断
        output_filename_hint (str): 输出 .xmind 文件名（含扩展名），默认由文件名推断
    
    示例：
        create_xmind_from_md("output/test_cases/额度服务V1.3.0版本_详细测试用例.md")
        create_xmind_from_md("output/test_cases/详细测试用例.md", output_dir="output/xmind")
    """
    ...
    # 6. 一次性生成所有用例
    hint = output_filename_hint
    if hint is None:
        if product_name and version:
            hint = "{}{}测试用例.xmind".format(product_name, version)
        elif filename:
            hint = os.path.splitext(filename)[0] + ".xmind"

    create_xmind_file_v2(
        all_cases,
        output_dir=output_dir,
        custom_path=custom_path,
        output_filename_hint=hint
    )
...
def main():
    ...
    if len(sys.argv) < 2:
        ...
        print("\n示例:")
        print("  python markdown_to_xmind.py output/test_cases/详细测试用例.md")
        print("  python markdown_to_xmind.py output/test_cases/额度服务V1.3.0版本_详细测试用例.md output/xmind")
        ...
        print("\n配置文件格式（JSON，用于中文路径）:")
        print('  {"md_file": "...", "output_dir": "...",')
        print('   "custom_path": "可选-脑图根节点标题", "output_filename": "可选-输出文件名.xmind"}')
        print("\n格式说明：")
    ...
    if sys.argv[1] == "--config":
        ...
        try:
            ...
            out_hint = config.get("output_filename")
            create_xmind_from_md(
                md_file,
                output_dir,
                custom_path=custom_path,
                output_filename_hint=out_hint,
            )
            return
